...
#include <map>           // For using std::map (key–value storage for campuses/sockets)
#include <mutex>         // For std::mutex to prevent data conflicts between threads
#include <cstring>       // For C‑style string functions like memset(), strlen(), strcmp()
#include <ctime>         // For handling time, timestamps, and ctime()
#include <unistd.h>      // For close(), read(), write(), sleep() — Unix system functions
#include <arpa/inet.h>   // For socket address structures and functions like inet_pton(), inet_addr()
#include <sys/socket.h>  // For creating sockets, bind(), connect(), send(), recv()
